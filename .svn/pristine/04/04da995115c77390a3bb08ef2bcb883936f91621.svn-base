...
long 	Size;
	...
	SetCtrlVal (painel_auto, AUTO_LED_MSAMPLE_1, master_sample_ok);
	SetCtrlVal (painel_auto, AUTO_LED_NEGSAMPLE_1, negativ_sample_ok);
...
void Grava_Hist_e_Status_das_Samples(int Max_Test, int berco)
{
	long 	Size;
    int  	Arquivo_Coleta , 
			cont, 
	...
	static char	modelo_ant[20]="xxx";
		...
		if(amostra_tipo_1 == MASTER_SAMPLE && dados_berco[berco].status_produto == PRODUTO_APROVADO)	
		{
			status = WriteLine(Arquivo_Coleta , STR_MAST_SAMP_OK , StringLength(STR_MAST_SAMP_OK));
			master_sample_ok = ON;
			
		}
		else if(amostra_tipo_1 == NEGATIVE_SAMPLE)
		{
			...
			{
				if(VerConsistNegatSample(1) == SEM_ERRO)
				{
		...
		if(amostra_tipo_1 == MASTER_SAMPLE)
		...
		if(dados_berco[berco].status_produto == PRODUTO_APROVADO)
		{
			Fmt( Buf , "%s<%s%s%s%s Id: %s", "Serial = ", dados_berco[berco].N_Serie_EndItem, "  - APROVADO  -  Operador(a): ", nomeCADASTRO, sprontuario ) ;            
		}
		else
		{
			Fmt( Buf , "%s<%s%s%s%s Id: %s", "Serial = ", dados_berco[berco].N_Serie_EndItem, "  - REPROVADO  -  Operador(a): ", nomeCADASTRO, sprontuario ) ;            
		}
		status = WriteLine(Arquivo_Coleta , Buf , StringLength(Buf));
		
		strcpy(Buf,STR_COMENT);                                           
		status = WriteLine(Arquivo_Coleta , Buf , StringLength(Buf));
		for (cont = 0 ; cont < Max_Test ; cont++)
		{ 
			strcpy(Buf,Teste[berco][cont].Nome);
	   	    Fmt(Buf , "%s<%s%s\t" , Buf ,",");
			if(Teste[berco][cont].LimBaixo <= 9999)
	   	    	Fmt(Buf , "%s<%s%f[p2]\t" , Buf , Teste[berco][cont].LimBaixo);	 
			else
	   	    	Fmt(Buf , "%s<%s%f[p1]\t" , Buf , Teste[berco][cont].LimBaixo);	 
	   	    Fmt(Buf , "%s<%s%s" , Buf ,",");
			if(Teste[berco][cont].Resultado <= 9999)
   	    		Fmt(Buf , "%s<%s%f[p2]\t" , Buf , Teste[berco][cont].Resultado);
			else
   	    		Fmt(Buf , "%s<%s%f[p1]\t" , Buf , Teste[berco][cont].Resultado);
			Fmt(Buf , "%s<%s%s" , Buf ,",");
			if(Teste[berco][cont].LimAlto <= 9999)
	   	    	Fmt(Buf , "%s<%s%f[p2]\t" , Buf , Teste[berco][cont].LimAlto);	 
			else
	   	    	Fmt(Buf , "%s<%s%f[p1]\t" , Buf , Teste[berco][cont].LimAlto);	 
	   	    Fmt(Buf , "%s<%s%s" , Buf ,",");
	   	    Fmt(Buf , "%s<%s%s" , Buf , Teste[berco][cont].Unidade);	 
	   	    Fmt(Buf , "%s<%s%s\t" , Buf ,",");
   	    	Fmt(Buf , "%s<%s%s" , Buf , Teste[berco][cont].Condicao);
			Fmt(Buf , "%s<%s%s\t" , Buf ,",");
	   	    Fmt(Buf , "%s<%s%f[p3]" , Buf , Teste[berco][cont].Tempo_teste);
	   	    Fmt(Buf , "%s<%s%s" , Buf ," s");
	        status = WriteLine(Arquivo_Coleta , Buf , StringLength(Buf));
   	    }
        Fmt(Buf , "%s<%s%s" , "Produto SW level = ",dados_berco[berco].sw_version);
        status = WriteLine(Arquivo_Coleta , Buf , StringLength(Buf));
        Fmt(Buf , "%s<%s%s" , "Versão SW de teste = ",VERSAO);
        status = WriteLine(Arquivo_Coleta , Buf , StringLength(Buf));
        Fmt(Buf , "%s<%s%f[p2]" , "Tempo de teste = ",dados_berco[berco].tempo_total);
        status = WriteLine(Arquivo_Coleta , Buf , StringLength(Buf));
        Data_e_Hora (Data, Dtimer, DmesI);
...
int RegistraNegatSample(int berco)
{
char  	nome_arquivo[200],
...
long 	Size;
	...
	strcat(Buf,dados_berco[berco].N_Serie_EndItem);
	strcat(Buf,",");
	...
	for (cont = 0 ; cont < max_teste ; cont++)
	{ 
		if(cont == (max_teste-1))
			Fmt(Buf , "%s<%s%i" , Buf ,Teste[berco][cont].Numero);
		else
			Fmt(Buf , "%s<%s%i%s" , Buf ,Teste[berco][cont].Numero, ",");
	}
	status = WriteLine(file_handle , Buf , StringLength(Buf));
	////////// registra Status /////////////////////////
	strcpy(Buf, "Status ,");
	...
	{ 
		if(cont == (max_teste-1))
			...
			Fmt(Buf , "%s<%s%s%s" , Buf , Teste[berco][cont].Condicao,",");
    }
    status = WriteLine(file_handle , Buf , StringLength(Buf));
	...
	{ 
		if(cont == (max_teste-1))
			...
			Fmt(Buf , "%s<%s%f[p2]%s" , Buf , Teste[berco][cont].Resultado,",");
    }
    status = WriteLine(file_handle , Buf , StringLength(Buf));
	...
	{ 
		if(cont == (max_teste-1))
			...
			Fmt(Buf , "%s<%s%f[p2]%s" , Buf ,Teste[berco][cont].LimBaixo,",");
    }
	status = WriteLine(file_handle , Buf , StringLength(Buf));
	...
	{ 
		if(cont == (max_teste-1))
   	    	...
   	    	Fmt(Buf , "%s<%s%f[p2]%s" , Buf ,Teste[berco][cont].LimAlto, ",");
    }
    status = WriteLine(file_handle , Buf , StringLength(Buf));
...
int AutorizaCriarNegatSample(int berco)
{
char  	nome_arquivo[200], 
		sample_status[1000];
int   	file_handle,
...
long 	Size;
	...
	if(amostra_tipo_1 != NEGATIVE_SAMPLE)
	{
			MessagePopup ("Erro ao criar Negative Sample", 
	...
	if (status == 1)
	{
		...
		do
		{
			...
			if(status == -2)//fim do arquivo
				sair = ON;
			if(strstr(sample_status, dados_berco[berco].N_Serie_EndItem) != NULL)
			{
...
int Gerenciar_Samples(int qual_berco)
{
int status;

	if(!master_sample_ok && amostra_tipo_1 != MASTER_SAMPLE)
	{
		MessagePopup ( " PRODUTO PADRÃO", "MASTER SAMPLE NÃO PASSOU HOJE!" ) ;
		return -1;                        
	}
	else if(!negativ_sample_ok && amostra_tipo_1 == TESTE )
	{
		MessagePopup ( " AMOSTRA NEGATIVA", "AMOSTRA NEGATIVA NÃO PASSOU HOJE!" ) ;
		return -2;                        
	}
	else if(!negativ_sample_ok && amostra_tipo_1 == NEGATIVE_SAMPLE && strstr(ident_da_sample, dados_berco[qual_berco].N_Serie_EndItem) == NULL && !criar_negat_sample)
	{
	...
	{
		if(AutorizaCriarNegatSample(qual_berco) < 0)
			return -4;
	}
	else if(amostra_tipo_1 == NEGATIVE_SAMPLE)
	{
		if((status=AutorizaTestarNegSample(1)) < 0)
		{
			switch(status)
			{
...
int AutorizaTestarNegSample(int berco)
{
char  	nome_arquivo[200], 
...
long 	Size;
	...
	do
	{
		...
		if(status == -2)//fim do arquivo
			sair = ON;
		if(strstr(linha_da_tabela, dados_berco[berco].N_Serie_EndItem) != NULL)
		{
			neg_sample_present = ON;
		}
	}while(!sair && !neg_sample_present);
	...
	if(index_ant > 0)
	{
		...
		{ 
			...
			TestNSample[berco][cont].Numero = atoi(aux);
	    }
	}
	...
	{ 
		if(Teste[berco][cont].Numero != TestNSample[berco][cont].Numero )
		{
			erro_confronto = ON;
			break;
		}
...
int VerConsistNegatSample(int berco)
{
char  	nome_arquivo[200], 
		linha_da_tabela[2000],
		aux[300];
...
long 	Size;
	...
	if(index_ant > 0)
	{
		...
		{ 
			if(cont != (max_teste-1))
			{
				...
				if(index_post > 0)
				{
					CopyString (TestNSample[berco][cont].Condicao, 0, linha_da_tabela, index_ant+1, index_post-(index_ant+1));
					index_ant = index_post;
				}
			}
			else
			{
				CopyString (TestNSample[berco][cont].Condicao, 0, linha_da_tabela, index_ant+1, -1);
			}
	    }
	}
	if(index_ant < 0 || index_post < 0)
	...
	for (cont = 0 ; cont < max_teste ; cont++)
	{ 
		if(strcmp(Teste[berco][cont].Condicao,  TestNSample[berco][cont].Condicao) != 0)
		{
			erro_confronto = ON;
			break;
		}
	...
	if(erro_confronto)
	{
		...
		strcat(aux, Teste[berco][cont].Nome);
...
int LeDescTipo_AmosNeg(int berco)
{
char
	aux[50],
	prefix[10];
int
	...
	for(i=0; i < MAX_NUM_NEG_SAMPLES; i++)
	{
		CopyString(prefix, 0, negat_samples_registr[i].serial, 0, TAMANHO_PREFIXO_ENDITEM);
		DefModel(prefix, TAMANHO_BARCODE_ENDITEM, LER_BCR_ENDITEM, berco);
...
int file;
	...
	if(file < 0)
		return -1;
	WriteLine ( file, dados_berco[berco].descricao_modelo, strlen(dados_berco[berco].descricao_modelo) ) ;
	strcpy(dados_berco[berco].descricao_modelo_atual, dados_berco[berco].descricao_modelo);
	CloseFile ( file );
return 1;
}

// *********************** LeTipoAtual *****************************
int LeTipoAtual(int berco)
{
int file,
	status;
	
	strcpy(dados_berco[berco].descricao_modelo_atual,"");
	file = OpenFile( ARQ_TIPO_ATUAL , VAL_READ_WRITE, VAL_OPEN_AS_IS, VAL_ASCII ) ;
	if(file < 0)
		return -1;
	status = ReadLine (file,dados_berco[berco].descricao_modelo_atual , -1);
	if(status < 0 || strlen(dados_berco[berco].descricao_modelo_atual) < 2)
	{
		strcpy(dados_berco[berco].descricao_modelo_atual,"");
		CloseFile ( file );
		return -2;
