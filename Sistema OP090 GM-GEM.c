...
int CVICALLBACK ThreadSistema (void *parametro)
{
	...
	while (!finalizar_thread_sistema)
	{
		...
		{
			Status_Seguranca();
			// verifica Presença do Produto no Berço 1
			...
			// verifica Presença do Produto no Berço 2
...
int IniciaPortasDeSaida(char *mens_erro)
{
	...
	// Inicializa a porta de saída 'CARTAO_2'
	Delay_thread(0.15);
	...
	if(status != 0)
	{
		strcpy(mens_erro, "Cartão de SAIDA('CARTAO_2') endereço 0x42 não respondeu!\rErro na comunicação I2C do Sistema" );
		return FALHA;
	}
	else
	{
	...
	if(status != 0)
	{
		strcpy(mens_erro, "Cartão de SAIDA('CARTAO_3') endereço 0x44 não respondeu!\rErro na comunicação I2C do Sistema" );
		return FALHA;
	}
	else
	{
	...
	// Inicializa a porta de saída 'CARTAO_4'
	Delay_thread(0.15);
	...
	if(status != 0)
	{
		strcpy(mens_erro, "Cartão de SAIDA('CARTAO_4') endereço 0x46 não respondeu!\rErro na comunicação I2C do Sistema" );
		return FALHA;
	}
	else
	{
...
// Função: Trava()
//
// Descrição: Utilizada para travar a porta.
//
int Trava (int acao)
{
	...
	if (acao == ON)
	{
		// Verifica se o trava já esta avançada 
		if ( STATUS.Trava_Porta_Avc==OFF )
		{
			...
			{
				MessagePopup ("Condição incorreta", "Condição inválida para travar a porta!");
				return FALHA;
			}
			...
			{
				if(modo_manual)
					MessagePopup("Condição incorreta", "TimeOut na trava da porta!");
	...
	{
		// Verifica se o Conector_Berco_1 já esta recuado .
		if ( STATUS.Trava_Porta_Avc==ON )
		{
			// Verificação de segurança
			if(modo_manual && motions_clear_to_move == OFF)
			{
				MessagePopup ("Condição incorreta", "Condição inválida para mover a trava da porta!");
				return FALHA;
			}
			if(!status_emergencia  || sair_programa)
				return EMERG_ACIONADA;
...
//
// Função: MoveClampBerco1()
//
// Descrição: Utilizada para executar o movimento do Clamp_Berco_1.
//
int MoveClampBerco1 (int acao)
{
	...
	if (acao == ON)
	{
		// Verifica se o Clamp_Berco_1 já esta avançado 
		if ( STATUS.ClampAdv_1==OFF || STATUS.ClampRec_1==ON )
		{
			...
			if(modo_manual && motions_clear_to_move == OFF)
			{
				MessagePopup ("Condição incorreta", "Condição inválida para mover o Clamp_Berco_1!");
				return FALHA;
			}
			if(!status_emergencia  || sair_programa)
				return EMERG_ACIONADA;
			...
			//
			// Comando para avançar o Clamp_Berco_1
			//
			...
			{
				if(modo_manual)
					MessagePopup("Condição incorreta", "TimeOut no Clamp_Berco_1!");
				return FALHA_TIMEOUT;
			}
			ProcessSystemEvents();
		}
	...
	{
		// Verifica se o Clamp_Berco_1 já esta recuado .
...
//
// Função: MoveClampBerco2()
//
// Descrição: Utilizada para executar o movimento do Clamp_Berco_2.
//
int MoveClampBerco2 (int acao)
{
	...
	if (acao == ON)
	{
		// Verifica se o Clamp_Berco_2 já esta avançado 
		if ( STATUS.ClampAdv_2==OFF || STATUS.ClampRec_2==ON )
		{
			...
			if(modo_manual && motions_clear_to_move == OFF)
			{
				MessagePopup ("Condição incorreta", "Condição inválida para mover o Clamp_Berco_2!");
				return FALHA;
			}
			if(!status_emergencia  || sair_programa)
				return EMERG_ACIONADA;
			...
			//
			// Comando para avançar o Clamp_Berco_2
			//
			...
			{
				if(modo_manual)
					MessagePopup("Condição incorreta", "TimeOut no Clamp_Berco_2!"); 
				return FALHA_TIMEOUT;
			}
			ProcessSystemEvents();
		}
	...
	{
		// Verifica se o Clamp_Berco_2 já esta recuado .
...
//
// Função: MoveConectorBerco1()
//
// Descrição: Utilizada para executar o movimento do Conector_Berco_1.
//
int MoveConectorBerco1 (int acao)
{
	...
	if (acao == ON)
	{
		// Verifica se o Conector_Berco_1 já esta avançado 
		if ( STATUS.ConectorAdv_1==OFF )
		{
			...
			if(modo_manual && motions_clear_to_move == OFF)
			{
				MessagePopup ("Condição incorreta", "Condição inválida para mover o Conector_Berco_1!");
				return FALHA;
			}
			if(!status_emergencia  || sair_programa)
				return EMERG_ACIONADA;
			...
			//
			// Comando para avançar o Conector_Berco_1
			//
			...
			{
				if(modo_manual)
					MessagePopup("Condição incorreta", "TimeOut no Conector_Berco_1!");
				return FALHA_TIMEOUT;
			}
			ProcessSystemEvents();
		}
...
//
// Função: MoveConectorBerco2()
//
// Descrição: Utilizada para executar o movimento do Conector_Berco_2.
//
int MoveConectorBerco2 (int acao)
{
	...
	if (acao == ON)
	{
		// Verifica se o Conector_Berco_2 já esta avançado 
		if ( STATUS.ConectorAdv_2==OFF )
		{
			...
			if(modo_manual && motions_clear_to_move == OFF)
			{
				MessagePopup ("Condição incorreta", "Condição inválida para mover o Conector_Berco_2!");
				return FALHA;
			}
			if(!status_emergencia  || sair_programa)
				return EMERG_ACIONADA;
			...
			//
			// Comando para avançar o Conector_Berco_2
			//
			...
			{
				if(modo_manual)
					MessagePopup("Condição incorreta", "TimeOut no Conector_Berco_2!");
				return FALHA_TIMEOUT;
			}
			ProcessSystemEvents();
		}
	...
	{
		// Verifica se o Conector_Berco_2 já esta recuado .
...
// Função: MoveCombitac()
//
// Descrição: Utilizada para executar o movimento do Combitac.
//
int MoveCombitac (int acao)
{
	...
	if (acao == ON)
	{
		// Verifica se o Combitac já esta avançado 
		if ( STATUS.Combitac_Berco1==OFF || STATUS.Combitac_Berco2==OFF )
		{
			...
			if(modo_manual && motions_clear_to_move == OFF)
			{
				MessagePopup ("Condição incorreta", "Condição inválida para mover o Combitac!");
				return FALHA;
			}
			if(!status_emergencia  || sair_programa)
				return EMERG_ACIONADA;
			...
			//
			// Comando para avançar o Combitac
			//
			...
			{
				if(modo_manual)
					MessagePopup("Condição incorreta", "TimeOut no Combitac!");
				return FALHA_TIMEOUT;
			}
			ProcessSystemEvents();
		}
	...
	{
		// Verifica se o Combitac já esta recuado .
...
int TravaLigaCluster1 (int acao)
{
	...
	if (acao == ON)
	{
		// Verifica se o Clamp 1 já esta avançado 
		if (( STATUS.ClampRec_1==OFF && STATUS.ConectorAdv_1==OFF ) || !sair_programa )
		{
			// Verificação de segurança
			if(modo_manual && motions_clear_to_move == OFF)
			{
				MessagePopup ("Condição incorreta", "Condição inválida para mover o Clamp 1!");
				return FALHA;
			}
			if(!status_emergencia  || sair_programa)
				return EMERG_ACIONADA;
			...
			//
			// Comando para avançar o Clamp cluster 1
			//
			SAIDA.SolClampCluster_1 = ON;

			// Aguarda conclusão do avanço do clamp 1
			do
			{
				Delay_thread(0.100);
			}
			...
			{
				if(modo_manual)
					MessagePopup("Condição incorreta", "TimeOut no Conector 1!");
				return FALHA_TIMEOUT;
			}
			
			// Comando para avançar o conector 1
			SAIDA.SolConectorCluster_1 = ON;
	...
	{
		// Verifica se o Clamp 1 já esta recuado .
		if ( STATUS.ClampAdv_1==ON && STATUS.ConectorAdv_1==ON )
		{
			// Verificação de segurança
			if(modo_manual && motions_clear_to_move == OFF)
			{
				MessagePopup ("Condição incorreta", "Condição inválida para mover o Conector!");
				return FALHA;
			}
			if(!status_emergencia  || sair_programa)
				return EMERG_ACIONADA;
			...
			{
				if(modo_manual)
					MessagePopup("Condição incorreta", "TimeOut no Conector!");
				return FALHA_TIMEOUT;
			}
...
int TravaLigaCluster2 (int acao)
{
	...
	if (acao == ON)
	{
		// Verifica se o Clamp 2 já esta avançado 
		if (( STATUS.ClampRec_2 == OFF && STATUS.ConectorAdv_2 == OFF) || !sair_programa )
		{
			// Verificação de segurança
			if(modo_manual && motions_clear_to_move == OFF)
			{
				MessagePopup ("Condição incorreta", "Condição inválida para mover o Clmap 2!");
				return FALHA;
			}
			if(!status_emergencia  || sair_programa)
				return EMERG_ACIONADA;
			...
			//
			// Comando para avançar o Clamp 2
			//
			SAIDA.SolClampCluster_2 = ON;
			...
			{
				if(modo_manual)
					MessagePopup("Condição incorreta", "TimeOut no Conector 2!");
				return FALHA_TIMEOUT;
			}
	...
	{
		// Verifica se o Clamp já esta recuado .
		if ( STATUS.ClampAdv_2 == ON && STATUS.ConectorAdv_2 == ON )
		{
			// Verificação de segurança
			if(modo_manual && motions_clear_to_move == OFF)
			{
				MessagePopup ("Condição incorreta", "Condição inválida para mover o Conector 2!");
				return FALHA;
			}
			if(!status_emergencia  || sair_programa)
				return EMERG_ACIONADA;
			...
			{
				if(modo_manual)
					MessagePopup("Condição incorreta", "TimeOut no Clamp 2!");
				return FALHA_TIMEOUT;
			}
