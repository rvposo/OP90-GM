...
#define FALHA_NAO_ATUALIZ_MENS_CAN	-2	 //Foi solicitado um envio de mensagem CAN porém o SW não está atualizando as mensagens CAN
#define FALHA_TIMEOUT_CAN			-3   //Timeout ao tentar ler retorno da interface CAN
#define FALHA_CAN_SUB_FUNC_N_SUPP	-4   //Dado ou parametro incorreto para o serviço solicitado - Ver detalhes na Spec Part II(resp do cluster) 
#define FALHA_CAN_BUSY_REP_REQ		-5   //Dado ou parametro incorreto para o serviço solicitado - Ver detalhes na Spec Part II(resp do cluster) 
#define FALHA_CAN_COND_N_CORRECT	-6   //Dado ou parametro incorreto para o serviço solicitado - Ver detalhes na Spec Part II(resp do cluster) 
#define FALHA_CAN_INVALID_KEY		-7   //Dado ou parametro incorreto para o serviço solicitado - Ver detalhes na Spec Part II(resp do cluster)
#define FALHA_CAN_WRITING_FALURE	-8   //Dado ou parametro incorreto para o serviço solicitado - Ver detalhes na Spec Part II(resp do cluster)
#define FALHA_CAN_NUM_CANAIS		-9   //Não encontrado interface CAN no sistema
#define FALHA_CAN_OPEN_CANAL		-10  //Erro ao tentar abrir comunicação com a interface ou módulo CAN 
#define FALHA_CAN_CFG_CANAL			-11  //Erro ao tentar configurar a interface do canal CAN
#define FALHA_CAN_TIPO_INDEFINIDO	-12  //O tipo de interface CAN a ser utilizada na maquina é inválido
#define FALHA_CAN_NOT_IN_DIAG		-13  //O cluster não está em modo diagnóstico, que é necessário para se rodar um serviço
#define FALHA_CAN_SERVICO_INVAL		-14  //O tipo de serviço solicitado no modo diagnóstico não é válido. Para serviços válidos consultar Spec partII
#define FALHA_CAN_MODE_ENTRY_INVAL	-15  //Modo ou tipo de sessão escolhida para entrar em diagnóstico é invalido - detalhes no serviço $10
#define FALHA_CAN_NOT_POSIT_RESP    -16  //A reposta do CAN diag não é a resposta positiva esperada
#define FALHA_CAN_TAMANHO_RESP      -17  //O número de bytes de reposta do CAN diag não é o valor esperado
#define FALHA_CAN_PARAM_ERROR       -18  //Dado ou parametro incorreto enviado para o serviço solicitado - Ver detalhes na Spec Part II(parametro p/ função)
...
#define FALHA_CAN_MODO_INCOMPATIVEL	-21  //Modo diagnóstico não é compativel com serviço solicitado
#define FALHA_INDEX_LKEY_ASC_OVER	-22  //O valor do indexador para leitura do arquivo ASC do live key ultrapassa o máximo permitido para o tamanho do arquivo
...
/***************** Enumeracao para tabelas de calibração dos gauges ********************/
typedef enum
{
	VELO_CAN_MIN	 	, 
	VELO_CAN_MAX	 	, 
...
/********** Enumeracao para pontos de calibração/verificação chaplets dos gauges ********************/
typedef enum
{
	//velocimetro
	CHAP_PONTO_0_KPH , 
